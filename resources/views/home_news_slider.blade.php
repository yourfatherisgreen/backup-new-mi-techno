...
<head>
    ...
    <!-- 
      PENTING: Kita butuh 'Typography'
      Ini yang bikin konten dari Filament (bold, list, dll) jadi rapi
    -->
...
<body class="font-poppins text-gray-800 bg-gray-50">
    ...
    <!-- HEADER RESPONSIVE (Sama kayak di 'home') -->
        ...
        <div class="container max-w-6xl mx-auto px-4 flex flex-col md:flex-row gap-12">
            ...
            <div class="w-full md:w-2/3 fade-in">
                ...
                <div class="w-full h-auto md:h-[400px] rounded-xl overflow-hidden shadow-lg mb-8">
                    <img src="{{ asset('storage/' . ($news->cover_image ?? 'default.png')) }}" alt="{{ $news->title }}" class="w-full h-full object-cover">
                </div>
                
                <!-- 
                  ISI KONTEN (DARI FILAMENT)
                  'prose' dari Tailwind Typography bikin ini jadi rapi otomatis
                -->
                        ...
                        @foreach($recent_posts as $recent)
                            <a href="{{ route('news.show', $recent->slug) }}" class="flex items-center gap-4 group">
                                <div class="w-24 h-20 rounded-lg overflow-hidden flex-shrink-0">
                                    <img src="{{ asset('storage/' . ($recent->cover_image ?? 'default.png')) }}" alt="{{ $recent->title }}" class="w-full h-full object-cover transition duration-300 group-hover:scale-110">
                                </div>
                                <div>
